...
fn positive_param(value: f64) -> f64 {
    value.abs().max(PARAM_EPS)
}

fn non_zero_param(value: f64) -> f64 {
...
fn eval_four_pl(a: f64, b: f64, c: f64, d: f64, x: f64) -> f64 {
    let c_safe = positive_param(c);
    let ratio = positive_x(x) / c_safe;
    d + (a - d) / (1.0 + ratio.powf(b))
}

fn eval_five_pl(a: f64, b: f64, c: f64, d: f64, m: f64, x: f64) -> f64 {
    let c_safe = positive_param(c);
    let m_safe = positive_param(m);
    let ratio = positive_x(x) / c_safe;
    d + (a - d) / (1.0 + ratio.powf(b)).powf(m_safe)
}

fn eval_michaelis_menten(vmax: f64, km: f64, x: f64) -> f64 {
    let denominator = x + km;
    vmax * x / non_zero_param(denominator)
}

fn eval_exponential_basic(a: f64, b: f64, c: f64, x: f64) -> f64 {
    a + b * (-c * x).exp()
}

fn eval_exponential_linear(a: f64, b: f64, c: f64, d: f64, x: f64) -> f64 {
    a * (b * x).exp() + c * x + d
}

fn eval_exponential_half_life(a: f64, b: f64, c: f64, x: f64) -> f64 {
    let c_safe = positive_param(c);
    a + b / 2.0_f64.powf(x / c_safe)
}

fn eval_falling_exponential(y0: f64, v0: f64, k: f64, x: f64) -> f64 {
    let k_safe = non_zero_param(k);
    let one_minus_exp = -(-k_safe * x).exp_m1();
    y0 - (v0 / k_safe) * one_minus_exp
}

fn eval_power(a: f64, b: f64, x: f64) -> f64 {
    a * positive_x(x).powf(b)
}

fn eval_gaussian(a: f64, b: f64, c: f64, x: f64) -> f64 {
    let c_safe = positive_param(c);
    let delta = x - b;
    a * (-(delta * delta) / (2.0 * c_safe * c_safe)).exp()
}

fn eval_arrhenius(a: f64, b: f64, x: f64) -> f64 {
    let x_safe = positive_x(x);
    a * (b / x_safe).exp()
}

fn eval_inverse(a: f64, b: f64, x: f64) -> f64 {
    a + b / positive_x(x)
}

fn eval_logistic(a: f64, b: f64, c: f64, x: f64) -> f64 {
    let exponent = -b * (x - c);
    a / (1.0 + exponent.exp())
}
...
fn eval_bi_exponential(a1: f64, k1: f64, a2: f64, k2: f64, c: f64, x: f64) -> f64 {
...
fn eval_arctangent_step(a: f64, b: f64, c: f64, d: f64, x: f64) -> f64 {
    a * (b * (x - c)).atan() + d
}
...
fn eval_softplus(a: f64, b: f64, c: f64, d: f64, x: f64) -> f64 {
    a * softplus(b * (x - c)) + d
}

fn eval_lorentzian(a: f64, x0: f64, gamma: f64, c: f64, x: f64) -> f64 {
    let gamma_safe = positive_param(gamma);
    let u = (x - x0) / gamma_safe;
    c + a / (1.0 + u * u)
}

fn eval_natural_log(a: f64, b: f64, x: f64) -> f64 {
    let x_safe = positive_x(x);
    let b_safe = positive_param(b);
    a * (x_safe / b_safe).ln()
}
...
impl CurveFamily {
    ...
    pub(crate) fn evaluate_raw(self, params: &[f64], x: f64) -> f64 {
        match self {
            ...
            | Self::Septic
            | Self::Octic
            | Self::Nonic => eval_polynomial(params, x),
            ...
            Self::FivePl => eval_five_pl(params[0], params[1], params[2], params[3], params[4], x),
            Self::MichaelisMenten => eval_michaelis_menten(params[0], params[1], x),
            Self::ExponentialBasic => eval_exponential_basic(params[0], params[1], params[2], x),
            Self::ExponentialLinear => {
                eval_exponential_linear(params[0], params[1], params[2], params[3], x)
            }
            Self::ExponentialHalfLife => {
                eval_exponential_half_life(params[0], params[1], params[2], x)
            }
            Self::FallingExponential => {
                eval_falling_exponential(params[0], params[1], params[2], x)
            }
            Self::HyperbolicTangent => {
                eval_hyperbolic_tangent(params[0], params[1], params[2], params[3], x)
            }
            Self::ArctangentStep => {
                eval_arctangent_step(params[0], params[1], params[2], params[3], x)
            }
...
impl CurveParams {
    ...
    pub fn evaluate(&self, x: f64) -> f64 {
        match self {
            Self::Linear { a, b } => eval_polynomial(&[*a, *b], x),
            Self::Quadratic { a, b, c } => eval_polynomial(&[*a, *b, *c], x),
            Self::Cubic { a, b, c, d } => eval_polynomial(&[*a, *b, *c, *d], x),
            Self::Quartic { a, b, c, d, e } => eval_polynomial(&[*a, *b, *c, *d, *e], x),
            Self::Quintic { a, b, c, d, e, f } => eval_polynomial(&[*a, *b, *c, *d, *e, *f], x),
            Self::Sextic {
                a,
                b,
                c,
                ...
                f,
                g,
            } => eval_polynomial(&[*a, *b, *c, *d, *e, *f, *g], x),
            Self::Septic {
                a,
                b,
                c,
                ...
                g,
                h,
            } => eval_polynomial(&[*a, *b, *c, *d, *e, *f, *g, *h], x),
            Self::Octic {
                a,
                b,
                c,
                ...
                h,
                i,
            } => eval_polynomial(&[*a, *b, *c, *d, *e, *f, *g, *h, *i], x),
            Self::Nonic {
                a,
                b,
                c,
                ...
                i,
                j,
            } => eval_polynomial(&[*a, *b, *c, *d, *e, *f, *g, *h, *i, *j], x),
            ...
            Self::FivePl { a, b, c, d, m } => eval_five_pl(*a, *b, *c, *d, *m, x),
            Self::MichaelisMenten { vmax, km } => eval_michaelis_menten(*vmax, *km, x),
            Self::ExponentialBasic { a, b, c } => eval_exponential_basic(*a, *b, *c, x),
            Self::ExponentialLinear { a, b, c, d } => eval_exponential_linear(*a, *b, *c, *d, x),
            Self::ExponentialHalfLife { a, b, c } => eval_exponential_half_life(*a, *b, *c, x),
            Self::FallingExponential { y0, v0, k } => eval_falling_exponential(*y0, *v0, *k, x),
            Self::HyperbolicTangent { a, b, c, d } => eval_hyperbolic_tangent(*a, *b, *c, *d, x),
            Self::ArctangentStep { a, b, c, d } => eval_arctangent_step(*a, *b, *c, *d, x),
